  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      ...
      >
        ...
        >
          Explore the World of <span className="text-primary-600 dark:text-primary-400">Physics</span>
        </motion.h1>
        <motion.p
          variants={itemVariants}
          className="text-xl text-gray-600 dark:text-gray-400 mb-10 max-w-2xl mx-auto"
        >
          A deep dive into NVIDIA PhysX, real-time simulations, and the mechanics of modern game development.
        </motion.p>
      ...
      <motion.section
        ...
        className="mt-32"
      >
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-12 text-center">Latest from the PhysX Series</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {POSTS.slice(0, 3).map((post) => (
            <Link key={post.id} to={`/blog/${post.id}`} className="group cursor-pointer">
              <div className="aspect-video bg-gray-100 dark:bg-gray-800 rounded-2xl mb-4 overflow-hidden shadow-lg">
                <img 
                  src={post.image} 
                  alt={post.title} 
